...
namespace Swag4Net.DiffTool.Client
{
   public static class ApiComparer
   {
         ...
         ComparisonContext context)
      {
         ...
         comparison = ComparisonHelpers.CompareBool(previous.AllowEmptyValue, actual.AllowEmptyValue, context,
            "parameter does not allow empty value any more", "parameter now allows empty value");
         if (comparison != null)
            yield return comparison;

         foreach (DiffResult diff in previous.Schema.CompareTo(actual.Schema, context))
         {
            yield return diff;
         }
         ...
         ComparisonContext context)
      {
         ...
         {
            bool stacked = false;
            if (previousSchema.Reference?.Id != null)
            {
               if (context.IsSchemaStacked(previousSchema.Reference.Id))
               {
                  yield break;
               }
               else
               {
                  stacked = context.PushSchema(previousSchema.Reference.Id);
               }
            }

            DiffResult? comparison = ComparisonHelpers.CompareBool(previousSchema.Deprecated,
               actualSchema.Deprecated, context.AppendAttribute(nameof(OpenApiSchema.Deprecated)),
            ...
            comparison = ComparisonHelpers.CompareBool(previousSchema.Nullable, actualSchema.Nullable,
               context.AppendAttribute(nameof(OpenApiSchema.Nullable)), "schema does not allow null any more",
               "schema allows null value from now");
            if (comparison != null)
               yield return comparison;

            comparison = ComparisonHelpers.CompareBool(previousSchema.ExclusiveMaximum,
               actualSchema.ExclusiveMaximum, context.AppendAttribute(nameof(OpenApiSchema.ExclusiveMaximum)),
            ...
            comparison = ComparisonHelpers.CompareBool(previousSchema.ExclusiveMinimum,
               actualSchema.ExclusiveMinimum, context.AppendAttribute(nameof(OpenApiSchema.ExclusiveMinimum)),
            ...
            comparison = ComparisonHelpers.CompareBool(previousSchema.ReadOnly, actualSchema.ReadOnly,
               context.AppendAttribute(nameof(OpenApiSchema.ReadOnly)), "value is now readonly",
            ...
            comparison = ComparisonHelpers.CompareBool(previousSchema.UniqueItems, actualSchema.UniqueItems,
               context.AppendAttribute(nameof(OpenApiSchema.UniqueItems)), "all elements are now unique",
               "element are not unique any more");
            if (comparison != null)
               yield return comparison;

            comparison = ComparisonHelpers.CompareBool(previousSchema.WriteOnly, actualSchema.WriteOnly,
               context.AppendAttribute(nameof(OpenApiSchema.WriteOnly)), "value is now writeonly",
            ...
            comparison = ComparisonHelpers.CompareBool(previousSchema.AdditionalPropertiesAllowed,
               ...
               context.AppendAttribute(nameof(OpenApiSchema.AdditionalPropertiesAllowed)),
            ...
            foreach (DiffResult diff in ComparisonHelpers.CompareScalar(previousSchema.Format, actualSchema.Format,
               ctx.AppendAttribute(nameof(OpenApiSchema.Format)), "schema format has changed"))
            {
               yield return diff;
            }

            foreach (DiffResult diff in ComparisonHelpers.CompareScalar(previousSchema.Maximum,
               actualSchema.Maximum, ctx.AppendAttribute(nameof(OpenApiSchema.Maximum)),
            ...
            foreach (DiffResult diff in ComparisonHelpers.CompareScalar(previousSchema.Minimum,
               actualSchema.Minimum, ctx.AppendAttribute(nameof(OpenApiSchema.Minimum)),
            ...
            foreach (DiffResult diff in ComparisonHelpers.CompareScalar(previousSchema.Pattern,
               actualSchema.Pattern, ctx.AppendAttribute(nameof(OpenApiSchema.Pattern)),
            ...
            foreach (DiffResult diff in ComparisonHelpers.CompareScalar(previousSchema.Type, actualSchema.Type,
               ctx.AppendAttribute(nameof(OpenApiSchema.Type)), "schema type has changed"))
            {
               yield return diff;
            }

            foreach (DiffResult diff in ComparisonHelpers.CompareScalar(previousSchema.MaxItems,
               actualSchema.MaxItems, ctx.AppendAttribute(nameof(OpenApiSchema.MaxItems)),
            ...
            foreach (DiffResult diff in ComparisonHelpers.CompareScalar(previousSchema.MinItems,
               actualSchema.MinItems, ctx.AppendAttribute(nameof(OpenApiSchema.MinItems)),
            ...
            foreach (DiffResult diff in ComparisonHelpers.CompareScalar(previousSchema.MaxLength,
               actualSchema.MaxLength, ctx.AppendAttribute(nameof(OpenApiSchema.MaxLength)),
            ...
            foreach (DiffResult diff in ComparisonHelpers.CompareScalar(previousSchema.MinLength,
               actualSchema.MinLength, ctx.AppendAttribute(nameof(OpenApiSchema.MinLength)),
            ...
            foreach (DiffResult diff in ComparisonHelpers.CompareScalar(previousSchema.MinProperties,
               actualSchema.MinProperties, ctx.AppendAttribute(nameof(OpenApiSchema.MinProperties)),
            ...
            foreach (DiffResult diff in ComparisonHelpers.CompareScalar(previousSchema.MultipleOf,
               actualSchema.MultipleOf, ctx.AppendAttribute(nameof(OpenApiSchema.MultipleOf)),
            ...
            foreach (DiffResult diff in CompareTo(previousSchema.Discriminator, actualSchema.Discriminator,
               ctx.AppendAttribute(nameof(OpenApiSchema.Discriminator))))
            {
               yield return diff;
            }
            ...
            foreach (DiffResult diff in previousSchema.Required.ToDictionary(x => x)
               .CompareTo(actualSchema.Required.ToDictionary(x => x),
                  p => ctx.AppendAttribute(nameof(OpenApiSchema.Required)), ComparisonHelpers.NopCompare))
            {
               yield return diff;
            }

            foreach (DiffResult diff in CompareEnum(previousSchema.Enum, actualSchema.Enum,
               ctx.AppendAttribute(nameof(OpenApiSchema.Enum))))
            {
               yield return diff;
            }

            foreach (DiffResult diff in previousSchema.Not.CompareTo(actualSchema.Not,
               ctx.AppendAttribute(nameof(OpenApiSchema.Not))))
            {
               yield return diff;
            }

            var newCtx = ctx.AppendAttribute(nameof(OpenApiSchema.OneOf));
            foreach (DiffResult diff in ToSchemaDictionary(previousSchema.OneOf)
               .CompareTo(ToSchemaDictionary(actualSchema.OneOf), s => newCtx, CompareTo))
            {
               yield return diff;
            }

            newCtx = ctx.AppendAttribute(nameof(OpenApiSchema.AllOf));
            foreach (DiffResult diff in ToSchemaDictionary(previousSchema.AllOf)
               .CompareTo(ToSchemaDictionary(actualSchema.AllOf), s => newCtx, CompareTo))
            {
               yield return diff;
            }

            newCtx = ctx.AppendAttribute(nameof(OpenApiSchema.AnyOf));
            foreach (DiffResult diff in ToSchemaDictionary(previousSchema.AnyOf)
               .CompareTo(ToSchemaDictionary(actualSchema.AnyOf), s => newCtx, CompareTo))
            {
               yield return diff;
            }

            foreach (DiffResult diff in previousSchema.Properties.CompareTo(actualSchema.Properties, ctx.AppendType,
               CompareTo))
            {
               yield return diff;
            }
      ...
      private static IDictionary<string, OpenApiSchema> ToSchemaDictionary(IEnumerable<OpenApiSchema> schemas)
      {
         var dico = new Dictionary<string, OpenApiSchema>();
         int idx = 1;
         foreach (OpenApiSchema schema in schemas)
         {
            ...
            {
               schemaId = schema.Reference.Id;
            }
            else
            {
               schemaId = $"Inline-{idx:0000}";
               ++idx;
            }

            dico.Add(schemaId, schema);
         }
         ...
         OpenApiDiscriminator? actualSchemaDiscriminator, ComparisonContext context)
      {
            ...
            ComparisonContext ctx)
         {
            foreach (DiffResult diff in ComparisonHelpers.CompareScalar(previous.PropertyName, actual.PropertyName,
               ctx.AppendType(nameof(OpenApiDiscriminator.PropertyName)), "discriminator property has changed"))
            {
               yield return diff;
            }

            var discriminatorCtx = ctx.AppendType("discriminator");
            foreach (DiffResult diff in previous.Mapping.CompareTo(actual.Mapping, _ => discriminatorCtx,
               (pmap, amap, c) => ComparisonHelpers.CompareScalar(pmap, amap, c.AppendType("mapping"),
